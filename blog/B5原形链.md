...
4. 第三次代码优化
   ...
   1. JavaScript中，许多情况下要使用对象，如果是对象独有的属性和方法，那么就把独有的属性和方法直接放到对象中，如果是共有的属性和方法，那么就在对象放`key: value`，如果对象是实例，那么`key`为`__proto__`，如果对象为类型，那么`key`为`prototype`和`__proto__`（一般不研究），`value`是一个heap的地址
