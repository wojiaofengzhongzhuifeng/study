...
- http://jenkins.rickricks.com/view/api-%E6%96%87%E6%A1%A3/job/bos3d-js-api-pipeline/
- http://jenkins.rickricks.com/view/api-%E6%96%87%E6%A1%A3/job/ViewerDemo-geo-v2-pipeline/
- 这两个需要确认是否正常
...
- bos3d 后端 api pdf 有问题  

-  bos3d 后端 api url 有问题 ?

- 为什么有一些库只能通过 require，不能通过 import 引入？

- ```
  docker run  -d -v /Users/raojj/Desktop/test1/html:/usr/share/nginx/html -v /Users/raojj/Desktop/default.conf:/etc/nginx/conf.d/default.conf -p 80:80 ae2feff98a0c # 使用镜像生成容器，并且使用服务器的配置文件和文件作为 nginx 而非 docker 内部的配置文件和文件
  ```
